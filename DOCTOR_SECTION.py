...
def get_drug_info(drug_name: str) -> dict:
    """
    Gets comprehensive drug information from the OpenFDA API including purpose and side effects.
    Returns a dictionary with purpose and adverse reactions or error messages.
    """
    url = f"https://api.fda.gov/drug/label.json?search=openfda.brand_name:{drug_name}&limit=1"
    try:
        response = requests.get(url, timeout=5)
        if response.status_code != 200:
            return {"error": f"API Error: {response.status_code}", "purpose": None, "side_effects": None, "contraindications": None, "warnings": None}
        data = response.json()
        results = data.get('results', [])
        if not results:
            return {"error": "No results found", "purpose": None, "side_effects": None, "contraindications": None, "warnings": None}
        result = results[0]
        purpose = result.get('purpose')
        purpose_text = "\n".join(purpose) if purpose else "Purpose not available"
        adverse_reactions = result.get('adverse_reactions')
        side_effects_text = "\n".join(adverse_reactions) if adverse_reactions else "Side effects not available"
        contraindications = result.get('contraindications')
        contraindications_text = "\n".join(contraindications) if contraindications else "Contraindications not available"
        warnings = result.get('warnings')
        warnings_text = "\n".join(warnings) if warnings else "Warnings not available"
        return {
            "error": None,
        ...
        return {"error": f"OpenFDA request failed: {str(e)}", "purpose": None, "side_effects": None, "contraindications": None, "warnings": None}
...
def analyze_drug_interactions(selected_medicines, patient_info):
    ...
    try:
        ...
        return DrugAlertAnalysis(alerts=[], safe_combinations=[], overall_risk_level="UNKNOWN")
...
def render_upload_stage():
    ...
    with col1:
        ...
        st.info("Recording functionality is not supported in this integration.")
    with col2:
                ...
                if st.button("🔄 Process Audio", type="primary", use_container_width=True):
                    with st.spinner("Processing audio... This may take a few moments."):
                        try:
                            ...
                            st.session_state.disease_suggestions = generate_disease_suggestions(patient_info, transcription)
                            st.session_state.medicine_suggestions = generate_medicine_suggestions(st.session_state.disease_suggestions)
                            st.session_state.drug_alerts = analyze_drug_interactions(st.session_state.medicine_suggestions, patient_info)
...
def render_alerts_stage():
    ...
    with col3:
        if st.button("📄 Generate Prescription", type="primary", use_container_width=True):
            ...
            if high_risk_alerts:
                st.warning("High-risk alerts detected! Please confirm you want to proceed.")
                if st.button("Confirm Prescription", key="confirm_prescription_with_alerts"):
                    st.session_state.final_prescription = {
                        ...
                        'doctor_suggestions': st.session_state.patient_info.get('Follow_Up_Actions', ['Follow up in 2 weeks']),
...
def render_prescription_stage():
    ...
    if st.session_state.final_prescription:
        prescription = st.session_state.final_prescription
        medicines_text = '<br>'.join([f"• {med['medicine']} (Reason: {med['reason']})" for med in prescription['medicines']]) if prescription['medicines'] else 'No medications prescribed'
        ...
        <div class="prescription-card">
            ...
            <div style="display: flex; justify-content: space-between; margin: 1rem 0;">
                <div>
                    ...
                    <strong>Age:</strong> {prescription.get('age', 'Not provided')}<br>
                    <strong>Gender:</strong> {prescription.get('gender', 'Not provided')}
                </div>
                <div>
        ...
        with col3:
            # Pre-process strings to escape LaTeX special characters
            ...
            medicines_latex = '\n'.join([f"\\item {escape_latex(med['medicine'])} (Reason: {escape_latex(med['reason'])})" for med in prescription.get('medicines', [])]) if prescription.get('medicines') else r'\item No medications prescribed'
            ...
            medical_history_latex = escape_latex(prescription.get('medical_history', 'Not provided'))
            previous_drug_history_latex = escape_latex(prescription.get('previous_drug_history', 'Not provided'))
            medical_tests_latex = escape_latex(prescription.get('medical_tests_advised', 'None advised'))
...
\textbf{Patient:} """ + escape_latex(prescription.get('patient_name', 'Not provided')) + r""" \\
\textbf{Age:} """ + escape_latex(prescription.get('age', 'Not provided')) + r""" \\
\textbf{Gender:} """ + escape_latex(prescription.get('gender', 'Not provided')) + r"""
\end{minipage}
\begin{minipage}[t]{0.45\textwidth}
\textbf{Date:} """ + escape_latex(prescription.get('date', datetime.now().strftime('%Y-%m-%d'))) + r""" \\
\textbf{Doctor:} Dr. [Doctor Name] \\
\textbf{License:} [License Number]
\end{minipage}
...
""" + medical_history_latex + r"""

\section*{Previous Drug History}
""" + previous_drug_history_latex + r"""
...
\begin{itemize}
""" + doctor_suggestions_latex + r"""
\end{itemize}

\section*{Advised Medical Tests}
""" + medical_tests_latex + r"""
...
"""
            st.download_button(
                label="📥 Download PDF",
                data=latex_content,
                file_name=f"prescription_{escape_latex(prescription.get('patient_name', 'patient'))}_{datetime.now().strftime('%Y%m%d')}.tex",
                mime="text/latex",
                use_container_width=True
            )
        with col4:
        ...
        with col4:
            consultation_time = len(st.session_state.transcription) * 0.5
...
def enhanced_main():
    ...
    <div class="main-header">
        ...
        <p style="color: #b3d9ff; text-align: center; margin: 0.5rem 0 0 0;">
            AI-Powered Medical Consultation Assistant v2.0
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        patient_info = {}
        patient_info['name'] = st.text_input("Patient Name", value=st.session_state.patient_info.get('Patient_Name', ''), key="sidebar_patient_name")
        col1, col2 = st.columns(2)
        with col1:
            patient_info['age'] = st.number_input("Age", min_value=0, max_value=120, value=st.session_state.patient_info.get('Age', 0), key="sidebar_patient_age")
        with col2:
            patient_info['gender'] = st.selectbox("Gender", ['Male', 'Female', 'Other'], 
                                                index=['Male', 'Female', 'Other'].index(st.session_state.patient_info.get('Gender', 'Male')) 
                                                if st.session_state.patient_info.get('Gender') else 0, key="sidebar_patient_gender")
        
        with st.expander("📞 Contact Information"):
            patient_info['phone'] = st.text_input("Phone", value=st.session_state.patient_info.get('phone', ''), key="sidebar_patient_phone")
            patient_info['email'] = st.text_input("Email", value=st.session_state.patient_info.get('email', ''), key="sidebar_patient_email")
            patient_info['emergency_contact'] = st.text_input("Emergency Contact", value=st.session_state.patient_info.get('emergency_contact', ''), key="sidebar_emergency_contact")
        
        with st.expander("🏥 Medical Information"):
            patient_info['medical_history'] = st.text_area("Medical History", value=st.session_state.patient_info.get('Previous_Medical_History', ''), key="sidebar_medical_history")
            patient_info['drug_history'] = st.text_area("Previous Drug History", value=st.session_state.patient_info.get('Previous_Drug_History', ''), key="sidebar_drug_history")
            patient_info['allergies'] = st.text_area("Allergies", value=st.session_state.patient_info.get('Allergies', ''), key="sidebar_allergies")
            patient_info['family_history'] = st.text_area("Family History", value=st.session_state.patient_info.get('Family_Medical_History', ''), key="sidebar_family_history")
        
        with st.expander("🏃 Lifestyle Information"):
            current_lifestyle = st.session_state.patient_info.get('Lifestyle_Details', {})
            patient_info['smoking'] = st.selectbox("Smoking Status", ['Never', 'Former', 'Current'], 
                                                  index=['Never', 'Former', 'Current'].index(current_lifestyle.get('smoking', 'Never')) 
                                                  if current_lifestyle.get('smoking') in ['Never', 'Former', 'Current'] else 0, key="sidebar_smoking")
            patient_info['alcohol'] = st.selectbox("Alcohol Use", ['None', 'Occasional', 'Regular', 'Heavy'], 
                                                  index=['None', 'Occasional', 'Regular', 'Heavy'].index(current_lifestyle.get('alcohol', 'None')) 
                                                  if current_lifestyle.get('alcohol') in ['None', 'Occasional', 'Regular', 'Heavy'] else 0, key="sidebar_alcohol")
            patient_info['exercise'] = st.selectbox("Exercise Frequency", ['None', 'Rarely', 'Weekly', 'Daily'], 
                                                   index=['None', 'Rarely', 'Weekly', 'Daily'].index(current_lifestyle.get('exercise', 'None')) 
                                                   if current_lifestyle.get('exercise') in ['None', 'Rarely', 'Weekly', 'Daily'] else 0, key="sidebar_exercise")
            patient_info['Lifestyle_Details'] = {'smoking': patient_info['smoking'], 'alcohol': patient_info['alcohol'], 'exercise': patient_info['exercise']}
