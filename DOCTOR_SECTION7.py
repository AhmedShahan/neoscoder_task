...
RATE = 16000
CHUNK = 1024
RECORD_SECONDS = 300  # Maximum recording time in seconds
            ...
            if not st.session_state.is_recording:
                ...
                if st.button("⏹️ Stop Recording", type="secondary", use_container_width=True):
                    ...
                    if audio_file and os.path.exists(audio_file):
                        with st.spinner("🔄 Processing recorded audio... This may take a few moments."):
                            try:
                                # Process the audio
                                transcription = process_audio(audio_file)
                                
                                if not transcription:
                                    st.error("❌ No transcription generated. Please try recording again.")
                                    return
...
# Lifestyle Information
        with st.expander("🏃 Lifestyle Information"):
            # Get current lifestyle details with default to empty dict if None
            current_lifestyle = st.session_state.patient_info.get('Lifestyle_Details') or {}
            
            # Handle smoking status
            smoking_options = ['Never', 'Former', 'Current']
            smoking_value = current_lifestyle.get('smoking') if current_lifestyle else 'Never'
            try:
                smoking_index = smoking_options.index(smoking_value) if smoking_value in smoking_options else 0
            ...
            # Handle alcohol use
            alcohol_options = ['None', 'Occasional', 'Regular', 'Heavy']
            alcohol_value = current_lifestyle.get('alcohol') if current_lifestyle else 'None'
            try:
                alcohol_index = alcohol_options.index(alcohol_value) if alcohol_value in alcohol_options else 0
            except (ValueError, TypeError):
                alcohol_index = 0
            ...
            exercise_options = ['None', 'Rarely', 'Weekly', 'Daily']
            exercise_value = current_lifestyle.get('exercise') if current_lifestyle else 'None'
            try:
                exercise_index = exercise_options.index(exercise_value) if exercise_value in exercise_options else 0
            except (ValueError, TypeError):
                exercise_index = 0
